...
## Unreleased
- Migrated grant and financial aid modeling from legacy static boolean flags (`finance.financial_aid`) to dynamic track-based funding options (`funding_type === "grant"`): added real grant programs and bilingual translations for Kazakhstani universities (SDU, AITU, AMU, IITU, Satbayev, KBTU, KazNU, ENU, Narxoz, KIMEP, KazNMU, Abai KazNPU), updated university detail page to dynamically render available grants via `getGrantsFromCategories()` with empty state fallback, refactored AI scoring and UniChance aid eligibility, aligned catalog badges and comparison table aid format, and updated data audit script and E2E test suites (`ai_scoring.py`, `universities.json`, `universities_translations.json`, `render-sections.js`, `university-detail-helpers.js`, `universities.js`, `audit_universities_data.py`, `Localization/eng`, `Localization/ru`, `tests/e2e/`).
