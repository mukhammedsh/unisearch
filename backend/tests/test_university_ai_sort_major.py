...
class UniversityAiSortMajorTests(unittest.TestCase):
    def setUp(self):
        with universities_router._AI_SORT_CACHE_LOCK:
            universities_router._AI_SORT_CACHE.clear()
        self.client = TestClient(app)

    def test_major_query_keeps_non_matching_programs_for_unifit_explanation(self):
        sorted_items = [{
            "id": "example-university",
            "name": "Example University",
            "matchData": {"missingProgram": True},
        }]

        with patch.object(universities_router.uni_service, "list_universities", return_value={"items": sorted_items}) as list_universities, patch.object(
            universities_router.ai_scoring_service,
            "sort_universities_ai",
            return_value=sorted_items,
        ) as sort_universities:
            response = self.client.post("/universities/ai-sort", json={
                "profile": {},
                "major": "Medicine",
                "page": 1,
                "limit": 24,
        ...
        self.assertEqual([item["id"] for item in returned_items], ["example-university"])
        self.assertTrue(returned_items[0]["matchData"]["missingProgram"])
        self.assertIsNone(list_universities.call_args.kwargs["major"])
        self.assertEqual(sort_universities.call_args.kwargs["profile"]["major"], "Medicine")

    def test_map_points_returns_only_compact_items_inside_requested_bounds(self):
        map_items = [
            {
    ...
    def test_map_projection_drops_card_and_detail_fields(self):
