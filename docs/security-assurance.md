...
| **A04: Insecure Design** | Architectural flaws | Documented system architecture (`docs/architecture.md`), RFC process for major changes (`GOVERNANCE.md`), and explicit data and API scope validation. |
...
| **A08: Software & Data Integrity Failures** | Malicious packages, untrusted releases | Hash-verified dependency locks (`package-lock.json`, `backend/requirements.lock`, and `backend/requirements-dev.lock`). Cryptographic attestation of releases via GitHub Sigstore (`actions/attest-build-provenance`). |
