...
- **Сетка и отступы (Layout & Spacing):**
  ...
  - **Один слой отступа (Padding Accumulation):** Не навешивать `padding` одновременно на родительскую секцию, её внутреннюю обертку и дочернюю карточку (эффект туннеля / сжатия). Отступ задаётся один раз на целевом уровне.
  - **Шкала 4/8px:** Все отступы строго кратны 4 или 8px (`4, 8, 12, 16, 20, 24, 32, 48, 64px`). Никаких случайных значений (`11px`, `13px`, `25px`, `27px`).
  - **Единая линия выравнивания:** Базовые горизонтальные отступы контейнера страницы должны совпадать у шапки, тулбаров, фильтров и контентной сетки — контент не должен «плясать» по левому краю.
  - **Мобильное сжатие отступов:** На экранах ≤768px и ≤480px внешние паддинги секций уменьшаются до 12–16px, а `gap` до 8–12px, предотвращая удушение полезной площади контента.
...
## 8. АНТИ-ВАЙБКОДИНГ (Zero-Vibe Engineering Protocol)
- **Первопричина вместо костылей:** Запрещено «чинить» баги наслоением бесконечных `if (x && x.y)` или слепыми обертками `try/catch`. Всегда устранять корень проблемы (Root Cause Analysis).
- **Запрет скрытия ошибок (No Silent Failure):** Запрещены пустые `catch`/`except` и фиктивные возвраты заглушек. Ошибка обязана либо корректно обрабатываться (с логированием и UI-фидбеком), либо пробрасываться дальше.
- **Запрет зомби-кода и дубликатов:** Запрещено оставлять закомментированные простыни кода, функции-дубликаты (`handleAction2()`) и забытые `console.log`. Перед созданием новой функции всегда проверять наличие аналогов в проекте.
- **Полнота состояний интерфейса:** Любой интерактивный компонент обязан иметь 4 состояния: Loading (индикатор/скелетон), Empty (нет данных), Error (с понятным описанием и кнопкой повтора), Disabled (блокировка повторных отправок).
- **Следование архитектурному контракту:** Запрещены `any`, отсутствие схем валидации (Zod/Pydantic) и хранение серверного состояния во временных in-memory словарях/переменных процесса.
- **Осознанный контроль (No Blind Commits):** Никаких слепых генераций и непроверенных диффов. Каждая строка должна быть понятна, архитектурно обоснована и проверена.
