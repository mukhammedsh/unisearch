...
- После успешного push всегда создай annotated tag формата `vX.Y.Z` на релизном commit (`git tag -a vX.Y.Z -m "UniSearch X.Y.Z"`) и запушь его (`git push origin vX.Y.Z`).
- После push commit и tag проверь GitHub Actions для последнего commit. Если `gh` недоступен, используй GitHub API; в финальном ответе укажи ссылки на runs и статус.
- Если Actions упали из-за внешней причины вроде billing/account issue или недоступных логов, не исправляй вслепую: укажи точную причину из GitHub annotation/API и следующий ручной шаг.
- Финальный ответ по релизу должен содержать commit hash, tag, push status, список запущенных проверок и итоговый статус Actions.
...
- Проект open-source и должен запускаться у разных пользователей без локально-зашитых путей, персональных настроек или привязки к конкретной машине разработчика.
- При изменениях учитывай разные ОС: не используй команды, пути, shell-синтаксис и зависимости, которые ломают Windows, macOS или Linux без явной необходимости и документации.
- Учитывай разные варианты хостинга и деплоя: локальный запуск, VPS, Docker, managed platforms и reverse proxy. Конфигурация должна задаваться через env/runtime config, а не через хардкод.
- Не добавляй личные данные, API-ключи, токены, IP-адреса и `.env` в репозиторий.
- Перед commit/push проверяй diff и staged files, чтобы личные данные, локальные пути, приватные URL, токены, ключи, cookies, дампы, логи и `.env` не попали в историю.
