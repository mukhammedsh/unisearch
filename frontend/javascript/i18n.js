...
const DICT = {
  eng: {
    ...
    "universities.slider.budget": "Budget 💰",
    "universities.slider.prestige": "Prestige 🏆",
    "universities.slider.balanced": "Balanced (50/50)",
    "universities.slider.budget_priority": "Strict Budget Priority",
    "universities.slider.prestige_priority": "Top Prestige Priority",
    "universities.slider.focus_budget": "Focus on Budget ({value}%)",
    "universities.slider.focus_prestige": "Focus on Prestige ({value}%)",
    ...
    "guide.unifit.p1":
      "{fit} is the ranking mode used in the Universities page when sort is set to AI Smart Sort. It prioritizes realistic options, not only famous names. The score combines academic feasibility from your profile, financial fit from budget and yearly costs, and prestige signals such as rank and selectivity.",
    "guide.unifit.p2":
      "The budget-prestige slider changes weighting, but it does not ignore constraints. Even with prestige priority, tracks that are far outside your profile are penalized. With budget priority, academically suitable options still rank above cheap but unrealistic choices.",
    "guide.unifit.p3":
    ...
    "guide.ml.p5":
      "When interests are provided, UniFit uses a hybrid formula: final_score = 0.7 * hard_score + 0.3 * ml_score. If interests are empty, sorting stays rule-based.",
    ...
    "profile.option.study_mode_hybrid": "Hybrid (Blended)",
  ...
  rus: {
    ...
    "universities.slider.budget": "Бюджет 💰",
    "universities.slider.prestige": "Престиж 🏆",
    "universities.slider.balanced": "Сбалансировано (50/50)",
    "universities.slider.budget_priority": "Приоритет бюджета",
    "universities.slider.prestige_priority": "Приоритет престижа",
    "universities.slider.focus_budget": "Фокус на бюджете ({value}%)",
    "universities.slider.focus_prestige": "Фокус на престиже ({value}%)",
    ...
    "guide.unifit.p1":
      "{fit} — режим ранжирования на странице Universities при выборе AI Smart Sort. Он отдает приоритет реалистичным вариантам, а не только известным названиям.",
    "guide.unifit.p2":
      "Слайдер бюджет/престиж меняет веса, но не игнорирует ограничения: неподходящие треки штрафуются, даже если выбран престиж.",
    "guide.unifit.p3":
    ...
    "guide.ml.p5":
      "Если интересы заполнены, используется гибридная формула: final_score = 0.7 * hard_score + 0.3 * ml_score. Иначе сортировка полностью rule-based.",
    ...
    "profile.option.study_mode_hybrid": "Гибридный",
  ...
  kz: {
    ...
    "universities.slider.budget": "Бюджет 💰",
    "universities.slider.prestige": "Бедел 🏆",
    "universities.slider.balanced": "Теңгерімді (50/50)",
    "universities.slider.budget_priority": "Бюджет басымдығы",
    "universities.slider.prestige_priority": "Бедел басымдығы",
    "universities.slider.focus_budget": "Бюджетке фокус ({value}%)",
    "universities.slider.focus_prestige": "Беделге фокус ({value}%)",
    ...
    "guide.unifit.p1":
      "{fit} — Universities бетінде AI Smart Sort таңдалғанда қолданылатын ранжирлеу режимі. Ол тек танымал атауларды емес, шынайы нұсқаларды алға қояды.",
    "guide.unifit.p2":
      "Бюджет/бедел слайдері салмақтарды өзгертеді, бірақ шектеулерді елемейді: профильге сай келмейтін тректер бедел басымдығында да айыпталады.",
    "guide.unifit.p3":
    ...
    "guide.ml.p5":
      "Interests толтырылғанда гибрид формула қолданылады: final_score = 0.7 * hard_score + 0.3 * ml_score. Толтырылмаса, сұрыптау тек rule-based.",
    ...
    "profile.option.study_mode_hybrid": "Гибрид",
